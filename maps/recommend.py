...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        restaurants_in_clusters = group_by_centroid(restaurants, old_centroids)
        centroids = [find_centroid(cluster) for cluster in restaurants_in_clusters]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    xtoy = zip(xs, ys)
    Sxy = sum([(s[0]-mean_x) * (s[1]-mean_y) for s in xtoy])
    b = Sxy / Sxx
    a = mean_y - b * mean_x
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            rates[name] = user_rating(user, name)
        else:
            rates[name] = predictor(restaurant)
    return rates
