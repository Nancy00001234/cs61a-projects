        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    self.ant = insect  #为了下一次添加的时候check是否可以再加ant,
                    #因为self.ant已经是container了，且其中装了一个ant，所以无法再添加；
                    #但是如果不改的话，self.ant是个普通的ant,如果加入container,还可以
                    #继续添加，那一个place里面就会出现3只ant，造成错误
                else:
...
class Insect(object):
    ...
    is_ant = False
    damage = 0
    watersafe = False  # by default False, means cannot go to a Water place
...
class Bee(Insect):
    ...
    name = 'Bee'
    damage = 1
    watersafe = True  # Bees can fly, so they  can pass a Water place
...
class Ant(Insect):
    ...
    food_cost = 0
    blocks_path = True  # Is True by default, except for NinjaAnt
    container = False   # Is False by default, except for BodyguardAnt
    ...
    def can_contain(self, other):
        ...
        # ↑This ant is container; This doesn't contain another ant; The other ant isn't container
        # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current_place, pace = self.place, 0
        while current_place != hive:
            if current_place.bees and self.min_range <= pace < self.max_range:
                return random_or_none(current_place.bees)
            current_place, pace = current_place.entrance, pace + 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            # reduce the armor of bees
            bees = self.place.bees[:]
            # index = 0 # index is used to access the bees lst that we are about to modify
            for bee in bees:
                bee.reduce_armor(self.damage)
                #index += 1
            # remove FireAnt itself
            self.place.remove_insect(self)
        # END Problem 5
...
class ScubaThrower(ThrowerAnt):
    ...
    implemented = True
    food_cost = 6
    watersafe = True  # can be placed on Water
...
class BodyguardAnt(Ant):
    ...
    implemented = True   # Change to True to view in the GUI
    food_cost = 4
    container = True  # Differ from other Ants
    # END Problem 9
...
# END Problem 13
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        Ant.__init__(self)
        self.is_queen = self.queen_number == 0
        self.doubled_ants = []
        self.queen_number += 1
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.is_queen:  # The true(first) queen
            ThrowerAnt.action(self, colony)  # Throws a leaf like ThrowerAnt
            place_behind = self.place.exit
            while place_behind is not None: # 以tunnel的end为终止条件
                ant = place_behind.ant
                if ant and ant not in self.doubled_ants:  # this place has ant
                    ant.damage *= 2
                    self.doubled_ants.append(ant)
                    if isinstance(ant, BodyguardAnt) and ant.ant and ant.ant not in self.doubled_ants:
                        ant.ant.damage *= 2
                        self.doubled_ants.append(ant.ant)
                place_behind = place_behind.exit

        else: # a imposor
            self.reduce_armor(self.armor) # Impostors only reduce their own armor to 0.
        # END Problem 13
